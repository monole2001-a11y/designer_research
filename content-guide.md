...
| 항목 | 내용 |
|------|------|
| 파일명 | `product-designer-outlook-2026.html` |
| 목적 | 프로덕트 디자이너 채용 시장 현황·전망·역량·학교 정보를 한 페이지에 종합 |
| 주요 독자 | 커리어 전환·취업 준비 중인 디자이너, 채용 담당자, 교육 기관 |
| 기준일 | 2026년 5월 25일 |
| 언어 | 한국어 (섹션 8–9 테이블 헤더·기업명 일부 영어) |
| 총 섹션 수 | 10개 섹션 + Hero + Sources |
...
```
Hero
├── 섹션 1  — 핵심 시장 지표           (통계 수치 6개)
├── 섹션 2  — 시장 현황 및 채용 트렌드  (인사이트 카드 4개)
├── 섹션 3  — AI가 직업에 미치는 영향   (차트 2개 + 위험/기회 카드)
├── 섹션 4  — 연봉 현황                (미국·한국 테이블)
├── 섹션 5  — 주니어~미드레벨 역량 심층 분석
│              (비교 카드 2개 + 포트폴리오 체크리스트 + 채용담당자 팁)
├── 섹션 6  — 직급별 취업 난이도 전망   (4행 테이블)
├── 섹션 7  — 프로덕트 디자인 변화 흐름 (타임라인 4개 항목)
├── 섹션 8  — 빅테크 기업별 레이오프 현황
│              (요약 수치 4개 + 기업 테이블 12행 + 지역별 카드)
├── 섹션 9  — 석사 지원자 학교 & 학과 가이드 (미국)
│              (학교 테이블 7행 + 연봉 바차트 + 전략 팁)
└── 섹션 10 — 종합 전략 제언           (summary-box)
Sources
```
